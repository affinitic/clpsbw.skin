...
# ### gestion des listing de news des plate-forme ###
    @memoize
    def getNewsPlateForme(self, PlateForme):
        catalog = getToolByName(aq_inner(self.context), 'portal_catalog')
        events = catalog(portal_type='News Item',
                         review_state=('external', 'internal'),
                         path={'query': 'plone/point-d-appui-assuetudes', 'depth': 1},
                         sort_on='Date',
                         sort_order='reverse')
        return events

    def getNewsIconURL(self, eventsBrain):
        """
    ...
    def addPublicKeywordsIfNeededAndGetPks(self, publicPksOrValues):
        ...
        for value in publicPksOrValues:
            try:
                ...
                # ici ne plus mettre publicTable mais la classe de initializer
...
### themes ###
    ...
    def getAllActiveThemeForRessource(self):
        """
        table pg theme
        recuperation de toutes les themes actif pour les rssources-outils
        """
        wrapper = getSAWrapper('clpsbw')
        session = wrapper.session
        query = session.query(Theme)
...
### experience ###
    ...
    def getExperienceFromInstitutionPorteur(self, institutionPk):
        ...
        listeAllExperience = self.getAllExperienceByEtat('publish')
...
### ressources ###
    ...
    def getAllActiveRessourceByThemeRessourcePk(self, themePk):
        """
        table pg ressource
        recuperation d'un ressource selon theme_pk
        """
        wrapper = getSAWrapper('clpsbw')
        session = wrapper.session
        query = session.query(LinkRessourceTheme)
    ...
    #    query = session.query(LinkRessourceOutil)
    #    query = query.filter(LinkRessourceOutil.experience_fk == experiencePk)
...
### institution_type ###
    def getAllInstitutionType(self, institutionTypePk=None):
        ...
        if not institutionTypePk:
            institutionTypePk = fields.get('institution_type_pk')
...
### assuetude for institution ###
    ...
    def getAllInstitutionAssuetudeActiviteProposee(self, cible=None):
        ...
        if cible:
            if cible == 'public':
                query = query.filter(InstitutionAssuetudeActiviteProposee.assuetude_activite_proposee_public == True)
            if cible == 'pro':
                query = query.filter(InstitutionAssuetudeActiviteProposee.assuetude_activite_proposee_pro == True)
        query = query.order_by(InstitutionAssuetudeActiviteProposee.assuetude_activite_proposee_num_ordre)
...
#### ADD ####
    ...
    def addTheme(self):
        ...
        theme_experience = getattr(fields, 'theme_experience', False)
        theme_ressource = getattr(fields, 'theme_ressource', False)
        theme_creation_date = self.getTimeStamp()
        theme_modification_date = self.getTimeStamp()
    ...
    def addRessourcePhoto(self, ressourceFk):
        """
        table pg link_ressource_clps
        ajout des clps proprietaire de la ressource
        """
    ...
    def addInstitution(self):
        ...
        institution_assuet_activite_entourage = getattr(fields, 'institution_assuet_activite_entourage', 'False')
...
#### UPDATE ####
    ...
    def updateTheme(self):
        ...
        for theme in themes:
            ...
            theme.theme_experience = theme_experience
            theme.theme_ressource = theme_ressource
    ...
    def updateInstitution(self):
        ...
        institution_assuet_intervention = getattr(fields, 'field.institution_assuet_intervention', 'False')
        institution_assuet_activite_entourage = getattr(fields, 'institution_assuet_activite_entourage', 'False')
        institution_plate_forme_sante_ecole = getattr(fields, 'institution_assuet_activite_entourage', 'False')
        institution_plate_forme_assuetude = getattr(fields, 'institution_plate_forme_assuetude', 'False')
        institution_plate_forme_sante_famille = getattr(fields, 'institution_plate_forme_sante_famille', 'False')
        institution_plate_forme_sante_environnement = getattr(fields, 'institution_plate_forme_sante_environnement', 'False')
        institution_listing_ressource_plate_forme_sante_ecole = getattr(fields, 'institution_listing_ressource_plate_forme_sante_ecole', 'False')
        ...
        institution.institution_assuet_activite_entourage = institution_assuet_activite_entourage
